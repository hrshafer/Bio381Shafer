...
```
The curve seems to match the simulated data better than my imported data. This means that the gamma distribution may not be the best probability distribution to describe the data. It's not as obviously bimodal as the first data set I went through in question 3, but there is a slight peak before the large peak which skews the curve a bit. Therefore, the model does not do a great job of simulating realistic data, but the curve patterns are at least a little bit lined up with what we may expect to see.